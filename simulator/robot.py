...
class Robot:
    ...
    def move_object(self, next_node):
        print(
            f"Object between {self.current_node} and {next_node} is being moved."
        )
    ...
    def check_next_nodes(
        ...
        for neighbor in neighbors:
            ...
            if any(
                check_connections.issubset(set(sublist))
                for sublist in obstacles["barrier"]
            ):
                print(
                    f"There is a movable object between node {self.current_node} and {neighbor}."
                )
                self.increase_weight_for_edge_with_obstacle(neighbor)
            if any(
                check_connections.issubset(set(sublist))
                for sublist in obstacles["missing_line"]
            ):
                print(
                    f"There is a missing line between connection {self.current_node} and {neighbor}."
                )
    ...
    def increase_weight_for_edge_with_obstacle(self, node):
        self.graph[self.current_node] = [
            {
                k: (v + OBJECT_WEIGHT if k == node else v)
        ...
        self.graph[node] = [
            {
                k: (v + OBJECT_WEIGHT if k == self.current_node else v)
                for k, v in neighbor.items()
            }
            for neighbor in self.graph[node]
        ]
        print(
            f"Weight between {self.current_node} and {node} has been raised by {OBJECT_WEIGHT}."
        )
    ...
    def remove_edge(self, neighbor):
        self.graph[self.current_node] = [
            item
            for item in self.graph[self.current_node]
            if not (isinstance(item, dict) and neighbor in item)
        ]
        self.graph[neighbor] = [
            item
            for item in self.graph[neighbor]
            if not (isinstance(item, dict) and self.current_node in item)
        ]
